...
uint8 		M[ROW][COL];
void img_pro::thin_step1(Matrix& img)
{
	uint16 		i, j;
    uint16 		ii, jj;
	...
	do
	{
		...
		for (i = 2; i < (ROW - 2); i++)
		{
			for (j = 2; j < (COL - 2); j++)
			{
				...
				{
					P1  = img.at(i - 1,j - 1);   P2  = img.at(i - 1,j	);		P3	= img.at(i - 1,j + 1);
					P4  = img.at(i	  ,j + 1);   P5  = img.at(i + 1,j + 1);		P6	= img.at(i + 1,j	);
					P7  = img.at(i + 1,j - 1);   P8  = img.at(i    ,j - 1);		P9	= img.at(i - 2,j - 2);
					P10 = img.at(i - 2,j - 1);   P11 = img.at(i - 2,j	 );		P12 = img.at(i - 2,j + 1);
					P13 = img.at(i + 1,j - 2);   P14 = img.at(i	   ,j - 2);		P15 = img.at(i - 1,j - 2);
					//------------------------------------------------------------------------------//
